...
Class FormSubmit
{
	...
	protected function setInfo($type,$initArray,$dbDispatchName,$validateDispatName)
	{
		...
		is_object($dbDispatchName) && $this->dbModel = $dbDispatchName;
		//$validateDispatName为false时，验证模块分发名使用数据库操作模块分发名值
	 ...
	 * @return boolean
	 */
	protected function exists($table,$existsArray,$existsWhere)
	{
		$type = $this->type;
		...
		foreach($existsArray as $existsDataKey => $existsDataValue) {
			if($type == 'insert') {
				...
				if(is_object($insertExistsSelect)) {
					$recordExistsOption = $insertExistsSelect;
				}
				else {
					//如果是类类型直接赋值
					...
					//如果是数组，转换好后赋值
					else if(is_array($existsWhere)){
						$select = new \FormSubmit\DbSql\DbSql($adapter,$table);
						$recordExistsOption = $select->getExistsSelectObject($existsWhere);
					}
					else {
						$this->setLog('existsWhere params is error',__LINE__);
						return false;
					}
				}
			}
	...
	public function paramsIsOk($initArray,$params,$dbDispatchName,$validateDispatName)
	{
	    ...
	    $return = true;
	    if(!is_object($dbDispatchName)) {
	        //如果数据库操作模块不是类，则准备使用本程序提供数据库操作方法，此处为操作表名赋值
	    	trim((string)$dbDispatchName) == '' && $return = false;
	    	$this->tableName = trim((string)$dbDispatchName);
	    }
	 ...
	 * @return boolean
	 */
	public function formSubmit($type,$params,$existsParams,$existsWhere,$dbDispatchName,$validateDispatName)
	{
		$dbReturn = true;
		...
		if($existsParams !== false) {
			...
			if($this->isCustomExists === false) {
			    ...
			    $exists = $this->exists($tableName,$existsArray,$existsWhere);
			}
			else {
